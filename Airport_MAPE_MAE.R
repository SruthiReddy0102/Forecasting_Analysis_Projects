...
library(tseries)
library(readr)
Airline <- read_csv("C:/Data Science/Assignments/Module-Forecasting/Codes/Airlines Data.csv")
View(Airline)


tssales <- ts(Airline$Passengers, frequency = 12, start = c(96))
View(tssales)
...
ma_model1 <- sma(train)
?sma
ma_pred <- data.frame(predict(ma_model1, h = 20))
ma_pred
...
hw_a
hwa_pred <- data.frame(predict(hw_a, n.ahead = 20))
...
hw_abg
hwabg_pred <- data.frame(predict(hw_abg, n.ahead = 20))
...
hw_na
hwna_pred <- data.frame(predict(hw_na, n.ahead = 20))
hwna_pred
plot(forecast(hw_na,h=20))
hwna_mape <- MAPE(hwna_pred$fit,test)*100


hw_nab <- HoltWinters(train, gamma = F)
hw_nab
hwnab_pred <- data.frame(predict(hw_nab, n.ahead = 20))
hwnab_pred
plot(forecast(hw_nab, h = 20))
hwnab_mape <- MAPE(hwnab_pred$fit, test)*100

hw_nabg <- HoltWinters(train)
hw_nabg
hwnabg_pred <- data.frame(predict(hw_nabg, n.ahead = 20))
hwnabg_pred
plot(forecast(hw_nabg, h = 20))
...
forecast_new <- data.frame(predict(new_model, n.ahead = 20))
forecast_new
########################################################################
#
#
...
plot(forecast(ses_a,n.ahead=20))
